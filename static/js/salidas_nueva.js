  ...
  cancelButtonColor: "#6c757d"   // gris Bootstrap
    ...
    if (typeof swal === "function") {
        return swal({
            ...
            icon: "warning",
            buttons: {
                cancel: { text: "No", value: false, visible: true, className: "btn btn-secondary", closeModal: true },
                confirm: { text: "Sí, ir a crear", value: true, visible: true, className: "btn btn-success", closeModal: true }
            }
        }).then(ok => {
            if (ok) {
                window.location.href = `${URL_MENU_BENEFICIARIOS}?q=${encodeURIComponent(nombre)}`;
...
async function buscarParroquias(dispararModalSiNoExiste = false, mostrarAutocomplete = false) {
    ...
    try {
        ...
        const data = await resp.json();
        if (!data.success) throw new Error(data.message || "Error en la búsqueda de parroquias");
...
function seleccionarBeneficiarioDesdeAutocomplete(item) {
    ...
    const inputTipoHidden = document.getElementById("tipoBeneficiarioSeleccionado");
    if (inputTipoHidden) inputTipoHidden.value = parroquiaSeleccionada.tipo_beneficiario;
    ...
    if (resumen) {
        ...
        if (parroquiaSeleccionada.municipio) text += ` - ${parroquiaSeleccionada.municipio}`;
        if (parroquiaSeleccionada.encargado) text += ` (Encargado: ${parroquiaSeleccionada.encargado})`;
...
async function cargarTipoSalida() {
    ...
    try {
        ...
        if (!dataOk) throw new Error("No se encontró un endpoint válido para tipos de salida.");

        const items = dataOk.items || [];
        items.forEach(t => {
...
async function buscarLotes() {
    ...
    try {
        ...
        const resp = await fetch(`/salidas/api/lotes-disponibles?${params.toString()}`);
        if (!resp.ok) throw new Error("Error HTTP " + resp.status);

        const data = await resp.json();
        if (!data.success) throw new Error(data.message || "Error en la consulta de lotes");
        ...
        if (window.Swal && typeof Swal.fire === "function") {
            ...
            swal("Error", "No se pudieron cargar los productos disponibles.", "error");
        }
    }
}
...
function agregarLoteDesdeBoton(e) {
    ...
    const yaExiste = detalleSalida.some(d => d.id_entrada_detalle === idEntradaDetalle);
    if (yaExiste) {
        if (window.Swal && typeof Swal.fire === "function") {
...
function manejarCambioCantidad(e) {
    ...
    const item = detalleSalida.find(d => d.id_entrada_detalle === idEntradaDetalle);
    if (!item) return;
    ...
    if (val > max) {
        ...
        input.value = val;
        if (window.Swal && typeof Swal.fire === "function") {
            Swal.fire("Aviso", "La cantidad no puede superar el stock disponible.", "warning");
        } else if (typeof swal === "function") {
            swal("Aviso", "La cantidad no puede superar el stock disponible.", "warning");
        }
    }
    ...
    const totalKgEl = document.getElementById("totalKgSalida");
    if (totalKgEl) {
        const total = detalleSalida.reduce((acc, d) => acc + Number(d.cantidad_kg || 0), 0);
...
function manejarQuitarDetalle(e) {
    ...
    const idEntradaDetalle = Number(tr.dataset.idEntradaDetalle);
    detalleSalida = detalleSalida.filter(d => d.id_entrada_detalle !== idEntradaDetalle);
...
async function guardarSalida() {
    ...
    if (!idSeleccionado) {
        if (window.Swal && typeof Swal.fire === "function") {
            Swal.fire("Falta beneficiario", "Seleccione primero una parroquia o fundación.", "warning");
        } else if (typeof swal === "function") {
            swal("Falta beneficiario", "Seleccione primero una parroquia o fundación.", "warning");
        }
        return;
    }
    ...
    if (!idTipoSalida) {
        if (window.Swal && typeof Swal.fire === "function") {
            Swal.fire("Falta tipo de salida", "Seleccione el tipo de salida.", "warning");
        } else if (typeof swal === "function") {
    ...
    if (!detalleSalida.length) {
        if (window.Swal && typeof Swal.fire === "function") {
            Swal.fire("Sin productos", "Agregue al menos un producto a la salida.", "warning");
        } else if (typeof swal === "function") {
            swal("Sin productos", "Agregue al menos un producto a la salida.", "warning");
        }
        return;
    }
    ...
    if (!detallesParaEnviar.length) {
        if (window.Swal && typeof Swal.fire === "function") {
            Swal.fire("Sin cantidades", "Todas las cantidades están en 0. Ajuste antes de guardar.", "warning");
        } else if (typeof swal === "function") {
            swal("Sin cantidades", "Todas las cantidades están en 0. Ajuste antes de guardar.", "warning");
        }
        return;
    }
...
if (!tipoBenef || !idBenef) {
  if (window.Swal && typeof Swal.fire === "function") {
    Swal.fire("Falta beneficiario", "Seleccione una parroquia o fundación con el botón Elegir.", "warning");
  } else if (typeof swal === "function") {
    swal("Falta beneficiario", "Seleccione una parroquia o fundación con el botón Elegir.", "warning");
  }
  return;
}
...
const payload = {
  ...
  detalles: detallesParaEnviar
    ...
    try {
        ...
        const data = await resp.json();
        if (!resp.ok || !data.success) throw new Error(data.message || "Error al registrar la salida.");
        ...
        if (window.Swal && typeof Swal.fire === "function") {
            ...
            swal({
                ...
                icon: "success",
                buttons: {
                    cancelar: { text: "Cerrar", value: "no", visible: true, closeModal: true },
                    ver: { text: "Ver comprobante", value: "si", visible: true, closeModal: true }
                }
            }).then((value) => {
