...
function agregarFilaSubcategoria(sub) {
  ...
  row.innerHTML = `
        <td class="col-id">${sub.id_subcategoria}</td>
        <td class="col-nombre">${sub.nombre}</td>
        <td class="col-descripcion">${sub.descripcion}</td>
        <td class="col-id-categoria">${sub.categoria_nombre}</td>
        <td>
            <button class="btn btn-sm btn-success me-2" onclick="abrirModalEditarSubcategoria(${sub.id_subcategoria})">
                <i class="bi bi-pencil-fill"></i>
            </button>
            <button class="btn btn-sm btn-danger" onclick="eliminarSubcategoria(${sub.id_subcategoria})">
...
formSubcategoria.addEventListener("submit", async function (e) {
  ...
  const descripcion = document
    .getElementById("descripcionSubcategoria")
    .value.trim();
  ...
  try {
    ...
    if (res.ok && data.success) {
      ...
      swal(
        "Error",
        data.message || "No se pudo crear la subcategoría",
        "error",
      );
    }
  } catch (error) {
...
async function abrirModalEditarSubcategoria(id) {
  try {
    ...
    document.getElementById("idSubcategoriaModal").value = sub.id_subcategoria;
    document.getElementById("nombreSubcategoriaModal").value = sub.nombre;
    document.getElementById("descripcionSubcategoriaModal").value =
      sub.descripcion;
    ...
    const modal = new bootstrap.Modal(
      document.getElementById("modalActualizarSubcategoria"),
    );
    modal.show();
  } catch (error) {
...
// ==============================
document
  .getElementById("formActualizarSubcategoria")
  .addEventListener("submit", async function (e) {
    e.preventDefault();

    const id = document.getElementById("idSubcategoriaModal").value;
    const nombre = document
      .getElementById("nombreSubcategoriaModal")
      .value.trim();
    const id_categoria = document.getElementById(
      "categoriaSubcategoriaModal",
    ).value;
    const descripcion = document
      .getElementById("descripcionSubcategoriaModal")
      .value.trim();
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        swal(
          "Error",
          data.message || "No se pudo actualizar la subcategoría",
          "error",
        );
      }
    } catch (error) {
...
async function eliminarSubcategoria(id) {
  const row = Array.from(tablaSubcategorias.rows).find(
    (r) => r.querySelector(".col-id").textContent == id,
  );
