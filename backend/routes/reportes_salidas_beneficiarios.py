...
def api_tipos_salida_reportes():
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "message": "Error al cargar tipos de salida"
        }), 500
...
# VISTA HTML
# =========================================================
@reportes_salidas_benef_bp.route(
    "/reportes/salidas/beneficiarios",
    methods=["GET"]
)
...
# =========================================================
@reportes_salidas_benef_bp.route(
    "/reportes/salidas/beneficiarios/data",
    methods=["GET"]
)
def data_reporte_salidas_beneficiarios():
    ...
    try:
        ...
        if col_p and col_f:
            ...
            familias_expr = (
                f"COALESCE(p.{col_p}, 0)"
            )

            familias_group = f"p.{col_p}"

        elif col_f and not col_p:

            familias_expr = (
                f"COALESCE(f.{col_f}, 0)"
            )

            familias_group = f"f.{col_f}"
        ...
        sql = f"""
            SELECT
                ...
                SUM(sd.cantidad_kg)
                    AS total_kg_categoria,

                SUM(
                    COALESCE(sd.valor_total, 0)
                ) AS total_valor_categoria
        ...
        for r in rows:
            ...
            kg_cat = _decimal_or_zero(
                r.get("total_kg_categoria")
            )

            val_cat = _decimal_or_zero(
                r.get("total_valor_categoria")
            )
            ...
            benef_map[key]["categorias"].append(
                {
                    ...
                    "total_valor_str":
                        format_money(val_cat),
                }
            )

            benef_map[key]["total_kg"] += float(
                kg_cat
            )

            benef_map[key]["total_valor"] += float(
                val_cat
            )
        ...
        for b in beneficiarios:
            ...
            b["total_kg_str"] = format_kg(
                Decimal(
                    str(b["total_kg"])
                )
            )

            b["total_valor_str"] = format_money(
                Decimal(
                    str(b["total_valor"])
                )
            )
        ...
        # KPIs
        # =====================================================
        total_parroquias = sum(
            1
            for b in beneficiarios
            if b["beneficiario_tipo"] == "Parroquia"
        )

        total_fundaciones = sum(
            1
            for b in beneficiarios
            if b["beneficiario_tipo"] == "Fundacion"
        )
        ...
        return jsonify(
            {
                ...
                "totales": {
                    ...
                    "total_kg":
                        float(tot_kg_general),

                    "total_kg_str":
                        format_kg(tot_kg_general),

                    "total_valor":
                        float(tot_val_general),
        ...
        print(
            "Error data_reporte_salidas_beneficiarios:",
            e
        )
...
# =========================================================
@reportes_salidas_benef_bp.route(
    "/reportes/salidas/beneficiarios/excel",
    methods=["GET"]
)
def reporte_salidas_beneficiarios_excel():
    ...
    try:
        ...
        if fecha_desde and fecha_hasta:
            ...
            filename = (
                "reporte_salidas_beneficiarios_"
            ...
            filename = (
                "reporte_salidas_beneficiarios_TODO.xlsx"
            )
        ...
        print(
            "Error reporte_salidas_beneficiarios_excel:",
            e
        )
