...
def data_reporte_salidas_listado():
    ...
    id_tipo_salida = request.args.get(
        "id_tipo_salida",
        type=int
    )

    beneficiario_tipo = (
        request.args.get("beneficiario_tipo") or ""
    ).strip()

    solo_con_beneficiario = (
        request.args.get("solo_con_beneficiario") or ""
    ).strip().lower()

    fecha_desde, fecha_hasta = parse_fechas(
        desde_str,
        hasta_str
    )
    ...
    if beneficiario_tipo in (
        ...
        if beneficiario_tipo == "Parroquia":
            ...
            filtros.append(
                "s.id_parroquia IS NOT NULL"
            )

        else:

            filtros.append(
                "s.id_fundaciones IS NOT NULL"
            )

    # =========================
    # SOLO CON BENEFICIARIO
    # =========================
    if solo_con_beneficiario == "1":

        filtros.append(
            "(s.id_parroquia IS NOT NULL "
            "OR s.id_fundaciones IS NOT NULL)"
        )
    ...
    if q:
        ...
        if q.isdigit():
            ...
            filtros.append(
                ...
                (
                    s.id_salida = %s
                    OR COALESCE(p.nombre, f.nombre) LIKE %s
                    OR COALESCE(
                        p.numero_documento,
                        f.numero_documento
                    ) LIKE %s
            ...
            filtros.append(
                """
                (
                    COALESCE(p.nombre, f.nombre) LIKE %s
                    OR COALESCE(
                        p.numero_documento,
                        f.numero_documento
                    ) LIKE %s
    ...
    try:
        ...
        sql = f"""
            SELECT
                ...
                CASE
                    WHEN s.id_parroquia IS NOT NULL
                        THEN 'Parroquia'

                    WHEN s.id_fundaciones IS NOT NULL
                        THEN 'Fundacion'
        ...
        for r in rows:
            ...
            items.append({
                ...
                "beneficiario_tipo":
                    r.get("beneficiario_tipo")
                    or "N/A",

                "beneficiario_nombre":
                    r.get("beneficiario_nombre")
                    or "",
        ...
        print(
            "Error data_reporte_salidas_listado:",
            e
        )

        return jsonify({
            "success": False,
            "message": "Error al generar el listado"
        }), 500
...
# =========================
@reportes_salidas_listado_bp.route(
    "/reportes/salidas/listado/detalle/<int:id_salida>",
    methods=["GET"]
)
def api_detalle_salida_listado(id_salida):
    ...
    try:
        ...
        cur.execute("""
            ...
            JOIN bodegas b
                ON b.id_bodega = sd.id_bodega

            WHERE sd.id_salida = %s

            ORDER BY
                p.nombre ASC,
                sd.fecha_vencimiento ASC
        """, (id_salida,))
        ...
        for d in det:
            ...
            detalles.append({
                ...
                "cantidad_kg":
                    _f(d.get("cantidad_kg")),

                "valor_unitario":
                    _f(d.get("valor_unitario")),

                "valor_total":
                    _f(d.get("valor_total")),
            })
        ...
        cab_out = {
            ...
            "fecha_salida":
                cab["fecha_salida"].strftime(
                    "%Y-%m-%d"
                )
                if cab.get("fecha_salida")
                else None,
            ...
            "tipo_salida":
                cab.get("tipo_salida") or "",

            "observacion":
                cab.get("observacion") or "",

            "beneficiario_tipo":
                cab.get("beneficiario_tipo")
                or "N/A",

            "beneficiario_nombre":
                cab.get("beneficiario_nombre")
                or "",

            "numero_documento":
                cab.get("numero_documento")
                or "",
        ...
        print(
            "Error api_detalle_salida_listado:",
            e
        )
...
# =========================
@reportes_salidas_listado_bp.route(
    "/reportes/salidas/listado/excel",
    methods=["GET"]
)
def excel_reporte_salidas_listado():
    ...
    # =========================
    desde_str = (
        request.args.get("desde") or ""
    ).strip()

    hasta_str = (
        request.args.get("hasta") or ""
    ).strip()
    ...
    beneficiario_tipo = (
        request.args.get("beneficiario_tipo")
        or ""
    ).strip()
    ...
    try:
        ...
        #
        # IMPORTANTE:
        # Esta consulta usa pq y fd para beneficiarios.
        # Por eso NO reutilizamos directamente where_clause,
        # ya que where_clause utiliza p y f.
        # ==================================================
        ...
        where_clause_detalle = (
            "WHERE "
            + " AND ".join(filtros_detalle)
        )
        ...
        cur.execute(f"""
            ...
            LEFT JOIN parroquias pq
                ON pq.id_parroquia =
                   s.id_parroquia

            LEFT JOIN fundaciones fd
                ON fd.id_fundaciones =
                   s.id_fundaciones
        ...
        print(
            "Error excel_reporte_salidas_listado:",
            e
        )
        ...
        cur.close()
        conn.close()
@reportes_salidas_listado_bp.route("/reportes/salidas/debug/<int:id_salida>", methods=["GET"])
def debug_salida(id_salida):
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=RealDictCursor)
