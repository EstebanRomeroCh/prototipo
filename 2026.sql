...
VALUES (
        'Administrador',
        'Control total del sistema, usuarios, donaciones y reportes.'
    ),
    (
        'Asistente',
        'Gestiona donaciones, inventarios y generación de certificados.'
    ),
    (
        'Auxiliar de Bodega',
        'Apoya en el registro y clasificación de productos donados.'
    );
...
SELECT * FROM bitacora_acciones ORDER BY fecha DESC LIMIT 10;
...
CREATE TABLE parroquias (
    ...
    estado ENUM('Activo', 'Inactivo') DEFAULT 'Activo',
    fecha_registro TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_parroquia_tipo_org FOREIGN KEY (id_tipo_organizacion) REFERENCES tipos_organizacion (id_tipo_organizacion) ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT fk_parroquia_tipo_doc FOREIGN KEY (tipo_doc_id) REFERENCES tipo_documento (id_tipo_doc) ON DELETE RESTRICT ON UPDATE CASCADE
);

CREATE TABLE fundacines (
    ...
    estado ENUM('Activo', 'Inactivo') DEFAULT 'Activo',
    fecha_registro TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_fundaciones_tipo_org FOREIGN KEY (id_tipo_organizacion) REFERENCES tipos_organizacion (id_tipo_organizacion) ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT fk_fundaciones_tipo_doc FOREIGN KEY (tipo_doc_id) REFERENCES tipo_documento (id_tipo_doc) ON DELETE RESTRICT ON UPDATE CASCADE
);
...
CREATE TABLE productos (
    ...
    CONSTRAINT fk_prod_subcategoria FOREIGN KEY (id_subcategoria) REFERENCES subcategorias (id_subcategoria) ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT fk_prod_unidad FOREIGN KEY (id_unidad) REFERENCES unidades_medida (id_unidad) ON DELETE RESTRICT ON UPDATE CASCADE
);
...
INSERT INTO
    tipo_entrada (nombre, descripcion)
VALUES (
        'Reagro',
        'Producto del Campo'
    );
...
ADD estado ENUM('Activo', 'Anulado') DEFAULT 'Activo';

ALTER TABLE entradas
ADD COLUMN total_peso_kg DECIMAL(10, 3) NOT NULL DEFAULT 0 AFTER observacion;

ALTER TABLE entradas
ADD COLUMN total_valor DECIMAL(12, 2) NOT NULL DEFAULT 0 AFTER total_peso_kg;
...
CREATE TABLE entrada_detalles (
    ...
    CONSTRAINT fk_ed_entrada FOREIGN KEY (id_entrada) REFERENCES entradas (id_entrada),
    CONSTRAINT fk_ed_producto FOREIGN KEY (id_producto) REFERENCES productos (id_producto),
    CONSTRAINT fk_ed_categoria FOREIGN KEY (id_categoria) REFERENCES categorias (id_categoria),
    CONSTRAINT fk_ed_unidad FOREIGN KEY (id_unidad) REFERENCES unidades_medida (id_unidad),
    CONSTRAINT fk_ed_bodega FOREIGN KEY (id_bodega) REFERENCES bodegas (id_bodega)
);
...
ALTER TABLE entrada_detalles
ADD COLUMN peso_total_kg DECIMAL(10, 3) NOT NULL DEFAULT 0 AFTER peso_unitario;

ALTER TABLE entrada_detalles
ADD COLUMN valor_total DECIMAL(12, 2) NULL AFTER valor_producto;
...
CREATE TABLE inventario_productos (
    ...
    CONSTRAINT fk_inv_producto FOREIGN KEY (id_producto) REFERENCES productos (id_producto) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE donaciones_monetarias (
    ...
    FOREIGN KEY (id_donante) REFERENCES donantes (id_donante) ON DELETE RESTRICT ON UPDATE CASCADE,
    FOREIGN KEY (id_metodo) REFERENCES metodos_donacion (id_metodo) ON DELETE RESTRICT ON UPDATE CASCADE
);
...
CREATE TABLE donacion_monetaria_archivos (
    ...
    tipo_mime VARCHAR(100), -- image/jpeg, application/pdf, etc.
    fecha_subida TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_dm_archivo_donacion FOREIGN KEY (id_donacion) REFERENCES donaciones_monetarias (id_donacion) ON DELETE CASCADE
);
...
INSERT INTO
    tipo_salida (nombre, descripcion)
VALUES (
        'Intercambio',
        'Intercambio con Bancos de alimentos'
    );
...
CREATE TABLE salidas (
    ...
    CONSTRAINT fk_sal_tipo_salida FOREIGN KEY (id_tipo_salida) REFERENCES tipo_salida (id_tipo_salida),
    CONSTRAINT fk_sal_parroquia FOREIGN KEY (id_parroquia) REFERENCES parroquias (id_parroquia)
);
...
ALTER TABLE salidas
ADD CONSTRAINT fk_salidas_fundaciones FOREIGN KEY (id_fundaciones) REFERENCES fundacines (id_fundaciones) ON DELETE RESTRICT ON UPDATE CASCADE;

ALTER TABLE salidas
ADD COLUMN id_acta_vencimiento INT NULL AFTER id_fundaciones;

ALTER TABLE salidas
ADD CONSTRAINT fk_salidas_acta_vencimiento FOREIGN KEY (id_acta_vencimiento) REFERENCES actas_vencimiento (id_acta) ON DELETE SET NULL;
...
CREATE TABLE salida_detalles (
    ...
    CONSTRAINT fk_sd_producto FOREIGN KEY (id_producto) REFERENCES productos (id_producto),
    CONSTRAINT fk_sd_categoria FOREIGN KEY (id_categoria) REFERENCES categorias (id_categoria),
    CONSTRAINT fk_sd_bodega FOREIGN KEY (id_bodega) REFERENCES bodegas (id_bodega),
    CONSTRAINT fk_sd_entrada_detalle FOREIGN KEY (id_entrada_detalle) REFERENCES entrada_detalles (id_detalle)
);
...
-- 1 a 1 con salidas
id_salida INT NOT NULL UNIQUE, fecha_acta DATE NOT NULL,
...
ADD CONSTRAINT fk_acta_usuario FOREIGN KEY (id_usuario_creador) REFERENCES usuarios (id_usuario) ON DELETE RESTRICT ON UPDATE CASCADE;

ALTER TABLE actas_vencimiento DROP COLUMN creado_por;
